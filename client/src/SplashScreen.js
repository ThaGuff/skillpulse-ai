...
export default function SplashScreen({ onStart }) {
  return (
    <div style={{ flex:1,display:'flex',flexDirection:'column',padding:'32px 24px env(safe-area-inset-bottom, 32px)',overflowY:'auto' }}>
      <div style={{ flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:28 }}>
        <div style={{ position:'relative',width:88,height:88 }}>
          ...
          <div style={{ position:'absolute',inset:0,borderRadius:'50%',background:C.accentLow,border:`1.5px solid ${C.accentHi}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:36 }}>⚡</div>
        </div>
        <div style={{ textAlign:'center',animation:'fadeUp 0.5s ease 0.15s both' }}>
          <div style={{ fontSize:11,fontWeight:700,letterSpacing:'0.22em',color:C.accent,marginBottom:10,fontFamily:'Syne, sans-serif' }}>SKILLPULSE AI</div>
          <h1 style={{ fontSize:30,fontWeight:800,color:C.text,margin:'0 0 12px',lineHeight:1.15,fontFamily:'Syne, sans-serif' }}>Know exactly<br />what to learn next</h1>
          <p style={{ fontSize:14,color:C.textMid,lineHeight:1.65,margin:0,maxWidth:290,marginInline:'auto' }}>AI scans your skill gaps and builds a personalized income roadmap — before someone else fills your spot.</p>
        </div>
        <div style={{ width:'100%',background:C.card,borderRadius:18,border:`1px solid ${C.border}`,overflow:'hidden',animation:'fadeUp 0.5s ease 0.3s both' }}>
          {FEATURES.map((f, i) => (
            <div key={i} style={{ display:'flex',gap:14,alignItems:'flex-start',padding:'14px 18px',borderBottom:i<FEATURES.length-1?`1px solid ${C.border}`:'none' }}>
      ...
      <div style={{ flexShrink:0,paddingTop:24,animation:'fadeUp 0.5s ease 0.45s both' }}>
        ...
        <p style={{ textAlign:'center',fontSize:11,color:C.muted,margin:'10px 0 0' }}>Free · 90 seconds · No sign-up needed</p>
      </div>
    </div>
  );
}
